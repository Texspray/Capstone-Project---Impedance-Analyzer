/* ******************************************************
 * main.cpp
 * Central de Comando do Analisador de Impedância (v3.0 - Com Si5351)
 *
 * - Integração com Si5351 para Clock Externo (frequências baixas).
 * - Lógica de MUX independente.
...
#include <math.h>
#include "AD5933.h"
#include "Si5351.h" // <--- BIBLIOTECA DO GERADOR DE CLOCK
...
// --- Configuração do Clock Externo (Si5351) ---
// Define a frequência que o Si5351 vai gerar para o pino MCLK do AD5933.
// 1 MHz é um bom valor para permitir medidas em baixas frequências.
unsigned long g_system_clock = 1000000; 
...
void setupSi5351(void); // <--- Configuração do Si5351
...
int main(void)
{
  ...
  send_uart_string("\r\n\r\n--- Analisador de Impedancia STM32 v3.0 (Si5351 Ext Clock) ---\r\n");

  if (MX_I2C1_Init()) 
  {
    send_uart_string("I2C OK. Inicializando hardware...\r\n");

    // 1. Configura e liga o Clock Externo (Si5351) ANTES de mexer no AD5933
    setupSi5351(); 
    
    // 2. Configura hardware padrão
    setPot(190);
    setDAC(2048); 
...
/**
 * @brief Configura o Si5351 para gerar o clock do sistema
 */
void setupSi5351(void)
{
    send_uart_string("Configurando Si5351 (Clock Externo)...\r\n");
    
    // Inicializa o Si5351
    if (Si5351::init()) 
    {
        // Configura CLK0 para a frequência definida (ex: 1 MHz)
        if (Si5351::set_freq(0, g_system_clock)) 
        {
            // Habilita a saída CLK0
            Si5351::enable_output(0, true); 
            
            sprintf(tx_buffer, "Si5351 OK: CLK0 gerando %lu Hz\r\n", g_system_clock);
            send_uart_string(tx_buffer);
            
            // IMPORTANTE: Informa a biblioteca AD5933 sobre a nova frequência de clock
            // para que os cálculos de frequência (Start Freq/Increment) sejam corretos.
            // (Requer que você tenha adicionado este método no AD5933.cpp/h)
            AD5933::setExtClockFrequency(g_system_clock);
        } 
        else 
        {
            send_uart_string("FALHA: Nao foi possivel definir a frequencia no Si5351.\r\n");
        }
    } 
    else 
    {
        send_uart_string("FALHA: Si5351 nao detectado no barramento I2C.\r\n");
    }
}
...
void parseCommand(char* cmd)
{
  ...
  else if (strcmp(cmd, "setmclk") == 0 && arg)
  {
    char* port_s = strtok(arg, ",");
    char* freq_s = strtok(NULL, ",");

    if (port_s && freq_s)
    {
      uint8_t port = (uint8_t)atoi(port_s);
      unsigned long freq = (unsigned long)atol(freq_s);

      // 1. Tenta configurar a frequência no Si5351
      if (Si5351::set_freq(port, freq))
      {
         // 2. Garante que a porta esteja habilitada
         Si5351::enable_output(port, true);

         // 3. IMPORTANTE: Atualiza a variável global e avisa a biblioteca AD5933
         // Se não fizer isso, o cálculo da frequência de varredura ficará errado!
         g_system_clock = freq;
         AD5933::setExtClockFrequency(g_system_clock);

         sprintf(tx_buffer, "OK: Si5351 CLK%u definido para %lu Hz\r\n", port, freq);
      }
      else
      {
         sprintf(tx_buffer, "FALHA: O Si5351 nao aceitou a frequencia (Limites?).\r\n");
      }
    }
    else
    {
       sprintf(tx_buffer, "FALHA: Formato. Use: setmclk,porta,freq\r\n");
    }
    send_uart_string(tx_buffer);
  }
...
void printConfig(void)
{
  ...
  sprintf(tx_buffer, "System Clock: %lu Hz (Si5351)\r\n", g_system_clock); send_uart_string(tx_buffer);
...
/**
 * @brief Roda a lógica de calibração
 */
void runCalibration(void)
{
  send_uart_string("Iniciando Calibracao (Modo Clock Externo)...\r\n");
  
  // Reset e configuração do AD5933 para usar o clock externo
  if (!AD5933::reset()) {
      send_uart_string("Falha no Reset AD5933.\r\n");
      return;
  }

  // Habilita Clock Externo no AD5933
  if (!AD5933::setInternalClock(false)) { 
      send_uart_string("Falha: Nao foi possivel ativar Clock Externo.\r\n");
      return;
  }

  // Garante que a biblioteca saiba qual é a frequência real do clock (1 MHz)
  AD5933::setExtClockFrequency(g_system_clock);

  // Configura parâmetros de varredura
  if (!(AD5933::setStartFrequency(g_start_freq) &&  
        AD5933::setIncrementFrequency(g_freq_incr) && 
        AD5933::setNumberIncrements(g_num_incr) &&   
        AD5933::setPGAGain(PGA_GAIN_X1))) 
  {
    send_uart_string("Falha: Erro na inicializacao dos parametros do AD5933!\r\n");
    return;
  }
...
/**
 * @brief Roda a varredura de frequência
 */
void runSweep(void)
{
  ...
  send_uart_string("Iniciando Varredura...\r\n");
  
  // Garante Clock Externo também na varredura
...
bool setDAC(uint16_t value)
{
  ...
  // Byte 1: Comando (0x40 = Write DAC Register)
  buffer[0] = 0x40;
  // Byte 2: MSB (D11 a D4)
  ...
  HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(&hi2c1, MCP4725_ADDRESS, buffer, 3, 100);
